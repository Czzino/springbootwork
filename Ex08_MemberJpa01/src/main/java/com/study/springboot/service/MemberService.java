...
public class MemberService {
	...
	public Member insert(Member member) {
		...
		Member rMember = memberRepository.save(member); // 내부에 정해진 함수 
		return rMember;
	}
	/*
	 * Optional<T> : NullPointException 발생을 방지하기 위해 사용
	 *				 기존의 반환 값에 타입T를 Optional<T> Wrapping하여, 
	 *				  	null 대신 Optional안에 빈 타입 객체를 돌려주는 기법
	 *	 ex)       member = memberRepository.findById(id);
	 	member.getId();  => NullPointException 발생
	 */	
